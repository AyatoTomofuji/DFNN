...
class FuzzyClassifier:
    ...
    def fit(self, X, y):
        ...
        rule_combinations = [np.random.randint(0, 4, self.n_features) for _ in range(200)]

        self.selected_rules = random.sample(rule_combinations, 200)

        for i, q in enumerate(self.selected_rules):
            print(f"Rule {i + 1}/{len(self.selected_rules)}: {q}")
            m = self.fit_rule(X, q)
